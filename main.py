...
for criterion, online in ordering_methods:
    ...
    # Step 6: Find the closest OSM nodes for the start and end points
    orig = ox.distance.nearest_nodes(G, X=G.nodes[list(G.nodes)[0]]['x'], Y=G.nodes[list(G.nodes)[0]]['y'])
    dest = ox.distance.nearest_nodes(G, X=G.nodes[list(G.nodes)[-1]]['x'], Y=G.nodes[list(G.nodes)[-1]]['y'])
